...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState: busters.GameState):
        ...
        ghosts_pos = [max(pos, key=pos.get) for pos in livingGhostPositionDistributions]
        
        # Distancer.getDistance(pos1, pos2):
        # Distancer.getDistanceOnGrid(pos1, pos2)
        # import pdb; pdb.set_trace()
        
        near_ghost = 100000
        for pos in ghosts_pos:
            if near_ghost > self.distancer.getDistance(pacmanPosition, pos):
                near_ghost_pos = pos
        
        dist = 10000
        for action in legal:
            successorPosition = Actions.getSuccessor(pacmanPosition, action)
            action_dist = self.distancer.getDistance(successorPosition, near_ghost_pos)
