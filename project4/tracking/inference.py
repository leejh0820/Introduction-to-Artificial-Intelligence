...
def constructBayesNet(gameState: hunters.GameState):
    ...
    edges = [(variables[0], variables[3]), (variables[0], variables[4]), (variables[1], variables[3]), (variables[2], variables[4])]
    
    from itertools import product
    temp = list(product(range(X_RANGE), range(Y_RANGE), repeat=1))
    # import pdb; pdb.set_trace()
    for variable in variables[:3]:
        variableDomainsDict[variable] = temp
    for variable in variables[3:]:
        variableDomainsDict[variable] = list(range(X_RANGE+Y_RANGE+MAX_NOISE-1))
    "*** END YOUR CODE HERE ***"
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        for i in eliminationOrder:
            factor, newFactor = joinFactorsByVariable(factor, i)
            if len(newFactor.unconditionedVariables()) > 1:
                eliminateFactor = eliminate(newFactor, i)
                factor.append(eliminateFactor)
...
class DiscreteDistribution(dict):
    ...
    def sample(self):
        ...
        # import pdb; pdb.set_trace()
        # weighted by the values associated with each key.
        self.normalize()
...
class InferenceModule:
    ...
    def getObservationProb(self, noisyDistance: int, pacmanPosition: Tuple, ghostPosition: Tuple, jailPosition: Tuple):
        ...
        if noisyDistance == None:
            if manhattanDistance(jailPosition, ghostPosition):
                return 0
            else:
                return 1
        elif manhattanDistance(jailPosition, ghostPosition) == 0:
            return 0
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for pos in self.allPositions: # self.beliefs.keys():
            self.beliefs[pos] = copy_beliefs[pos] * \
                self.getObservationProb(noisyDistance=observation, 
                                        pacmanPosition=gameState.getPacmanPosition(),
                                        ghostPosition=pos, 
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        for pos in self.allPositions:
            # def getPositionDistribution(self, gameState, pos, index=None, agent=None):
            temp[pos] = self.getPositionDistribution(gameState, pos)
        
        for pos in self.allPositions:
            self.beliefs[pos] = sum([copy_beliefs[pos_]*temp[pos_][pos] for pos_ in self.allPositions])
        "*** END YOUR CODE HERE ***"
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        shuffle = self.legalPositions.copy()
        for _ in range(self.numParticles//len(self.legalPositions)):
            random.shuffle(shuffle)
            self.particles.extend(shuffle)
            
        # import pdb; pdb.set_trace()
        
        self.particles.extend(random.sample(shuffle, k=self.numParticles%len(self.legalPositions)))
        # import pdb; pdb.set_trace()
    ...
    def getBeliefDistribution(self):
        ...
        # DiscreteDistribution(dict) -> normalize()
        temp = Counter()
        for pos in self.particles:
            temp[pos] += 1
        
        
        distribution = DiscreteDistribution(temp)
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        temp = self.getBeliefDistribution()
        
        for pos in temp.keys():
            temp[pos] *= self.getObservationProb(noisyDistance=observation,
                                                 pacmanPosition=gameState.getPacmanPosition(),
                                                 ghostPosition=pos,
                                                 jailPosition=self.getJailPosition())
...
class JointParticleFilter(ParticleFilter):
    ...
    def initializeUniformly(self, gameState):
        ...
        for i in range(self.numGhosts):
            super(JointParticleFilter, self).initializeUniformly(gameState)
            random.shuffle(self.particles)
            temp.append(self.particles)
        ...
        for j in range(len(temp[0])):
            ...
            self.particles.append(tuple(temp2))
        "*** END YOUR CODE HERE ***"
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for pos in temp.keys():
            for i in range(self.numGhosts):
                temp[pos] *= self.getObservationProb(noisyDistance=observation[i],
                                                     pacmanPosition=gameState.getPacmanPosition(),
                                                     ghostPosition=pos[i],
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            for i in range(self.numGhosts):
                # # def getPositionDistribution(self, gameState, pos, index=None, agent=None):
                newParticle[i] = self.getPositionDistribution(gameState=gameState, 
                                                              pos=oldParticle,
                                                              index=i,
                                                              agent=self.ghostAgents[i]).sample()
