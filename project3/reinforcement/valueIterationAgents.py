...
class ValueIterationAgent(ValueEstimationAgent):
    ...
    def runValueIteration(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        # mdp.getStates(): (x1, y1), (x2, y2)...
        # mdp.getPossibleActions(state): ['north' ..
        # self.iterations: int  (depth k)
        # self.discout: float
        # self.mdp.isTerminal('TERMINAL_STATE'): True
        ...
        for _ in range(self.iterations):
            ...
            for state in self.mdp.getStates():
                if self.mdp.isTerminal(state):
                    continue
                temp[state] = max([self.getQValue(state, act) 
    ...
    def computeQValueFromValues(self, state, action):
        ...
        for state_, prob in self.mdp.getTransitionStatesAndProbs(state, action):
            ...
            Q += prob * (reward + self.discount * self.values[state_])
